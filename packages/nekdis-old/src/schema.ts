...
export class Schema<S extends SchemaDefinition, M extends MethodsDefinition<S> = {}, P extends ParseSchema<S> = ParseSchema<S>> {
    ...
    #parse(schema: SchemaDefinition, topLevelIndex: boolean = false): ParsedSchemaDefinition {
        ...
        for (let i = 0, entries = Object.entries(schema), len = entries.length; i < len; i++) {
            ...
            if (key.startsWith("$")) throw new Error("Keys cannot start with '$'");

            if (typeof value === "string") {
                //@ts-expect-error Some people do not read docs
                if (value === "object") throw new Error("Type 'object' needs to use its object definition");
                //@ts-expect-error Some people do not read docs
                else if (value === "reference") throw new Error("Type 'reference' needs to use its object definition");
                //@ts-expect-error Some people do not read docs
                else if (value === "relation") throw new Error("Type 'relation' needs to use its object definition");
                //@ts-expect-error Some people do not read docs
                else if (value === "tuple") throw new Error("Type 'tuple' needs to use its object definition");
                else if (value === "array") value = { type: value, elements: "string", default: undefined, optional: false, sortable: false, index: topLevelIndex, separator: "|" };
                else if (value === "vector") {
                    if (this.options.dataStructure === "HASH") throw new Error("Vectors currently aren't working with hashes");
            ...
            if (typeof value.type === "undefined") throw new Error("Type not defined");
            ...
            if (value.type === "array") {
                ...
                value = <never> this.#fill(value, value.index ?? topLevelIndex);
                ...
                if (typeof value.elements === "undefined") throw new Error("Tuple needs to have at least 1 element");

                for (let j = 0, le = value.elements.length; j < le; j++) value.elements[j] = <never> this.#parse({ [j]: value.elements[j] }, value.index ?? topLevelIndex).data[j];

                value = <never> this.#fill(value, value.index ?? topLevelIndex);
            } else if (value.type === "reference") {
                if (typeof value.schema === "undefined") throw new Error("Type 'reference' lacks a schema which is needed to provide intellisense");
                references[key] = null;
                continue;
            } else if (value.type === "relation") {
                if (typeof value.schema === "undefined") throw new Error("Type 'relation' lacks a schema");
                ...
                } else value.meta = <never> this.#parse({ in: "string", out: "string" }, true).data;
                ...
                if (this.options.dataStructure === "HASH") throw new Error("Vectors currently aren't working with hashes");
                if (typeof value.algorithm === "undefined") throw new Error("'algorithm' is missing on the vector definition");
                if (typeof value.distance === "undefined") throw new Error("'distance' is missing on the vector definition");
                if (typeof value.vecType === "undefined") throw new Error("'vecType' is missing on the vector definition");
                if (typeof value.dim === "undefined") throw new Error("'dim' is missing on the vector definition");
    ...
    #fill(value: BaseField, topLevelIndex: boolean): BaseField {
        if (typeof value.default === "undefined") value.default = undefined;
        if (typeof value.optional === "undefined") value.optional = false;
